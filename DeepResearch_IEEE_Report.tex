...
\author{\IEEEauthorblockN{Saptarshi Kundu}
\IEEEauthorblockA{\textit{Independent Researcher} \\
\texttt{https://github.com/Saptarshie/DeepSearch}}}
...
\begin{equation}
\text{score}(r) = 1.0 + \alpha \cdot \mathbf{1}[\text{title} \in K]
                       + \beta  \cdot \mathbf{1}[\text{.pdf}]
                       + \gamma \cdot \mathbf{1}[|\text{snippet}| > 80]
\label{eq:scoring}
\end{equation}
...
\begin{algorithmic}[1]
\STATE $acc \gets [\,]$, \quad $tokens \gets 0$
\FOR{each node $n$ in DFS traversal of topic tree}
    \STATE $ctx \gets \texttt{overview.md}$ \textbf{or} $\texttt{information.md}$ of $n$
    \IF{$ctx \neq \emptyset$}
        \STATE $acc\text{.append}\!\left(\langle\text{path}(n),\, ctx\rangle\right)$
        \STATE $tokens \gets tokens + \lvert ctx \rvert / 4$
        \IF{$tokens \geq \tau$}
            \STATE $\text{\textbf{flush}}(acc)$ $\triangleright$ Single LLM call for batch
            \STATE $acc \gets [\,]$, \quad $tokens \gets 0$
        \ENDIF
    \ENDIF
\ENDFOR
\IF{$acc \neq [\,]$}
    \STATE $\text{\textbf{flush}}(acc)$ $\triangleright$ Flush remaining
\ENDIF
\end{algorithmic}
\end{algorithm}

where $\tau$ is the minimum accumulator threshold (default: 400 tokens). This batching strategy produces more natural prose by allowing the LLM to weave related subtopics together, while reducing the total number of API calls. Additionally, the \texttt{ReportBuilder} injects structured visual summaries---including Mermaid flowcharts for causal chains and decision trees---directly into the generated markdown to improve readability and topical navigation.
...
\subsection{Report Post-Processing}

To ensure that automatically generated Mermaid diagrams render correctly, a lightweight post-processor scans each \texttt{report-content.md} output for unquoted special characters (e.g., \texttt{\$}, \texttt{()}, \texttt{+}) inside node labels and wraps them in double quotes. This fixes the majority of syntax errors introduced by LLM-generated Mermaid blocks without requiring manual intervention.
...
Table~\ref{tab:eval_matrix} compares DeepResearch against a single-pass RAG baseline across eleven evaluation dimensions.

\begin{table*}[t]
\caption{Comprehensive Evaluation Matrix: DeepResearch vs. Single-Pass RAG}
\label{tab:eval_matrix}
\centering
\scriptsize
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Dimension} & \textbf{Single-Pass RAG} & \textbf{DeepResearch} & \textbf{Improvement} \\
\midrule
Search Queries Executed & 8 (planned only) & 8 + 6.3 follow-up & +79\% \\
Unique Sources Retrieved & 12.4 & 23.7 & +91\% \\
Documents Analyzed & 10 & 10 (\texttt{max\_docs} cap) & --- \\
...
Report Length (words) & $\sim$8,000-12000 & $\sim$100,000-200,000 & +800\% \\
Citations & $\sim$15 & $\sim$42 & +180\% \\
...
Total Runtime & $\sim$5\,min & $\sim$30\,min & +500\% \\
\bottomrule
\end{tabular}
\end{table*}
