...
```mermaid
flowchart TD
    subgraph Historical_Pattern["Historical Intervention Pattern"]
        A1[2008 Crisis] --> B1[$710B Section 13(3) Lending]
        A2[2020 Crisis] --> B2[$454-500B Liquidity Facilities]
        A3[2024 BTFP] --> B3[Interest Rate Loss Backstop]
        B1 --> C[~5 Year Cycle Established]
        B2 --> C
        B3 --> C
    end
    
    subgraph Risk_Vectors["Current Risk Vectors"]
        D[CRE Deterioration] --> E[Mid-tier Bank Stress]
        F[Repo Market Dependence] --> G[NYFed Unlimited Operations]
        H[Duration Mismatch] --> I[Securities Portfolio Losses]
        E --> J[Probability Assessment]
        G --> J
        I --> J
    end
    
    subgraph Fiscal_Constraint["Fiscal Capacity Constraint"]
        K[Mandatory Spending 73%] --> L[Revenue Consumption]
        M[Net Interest Doubling] --> L
        L --> N[Reduced Intervention Scale]
    end
    
    J --> O[Federal Intervention Probability: >80%]
    N --> P[Constraint on Response Scale]
...
```mermaid
flowchart TD
    A[Valuation Pressures + Borrowing Risks] --> B[Market Correction Trigger]
    B --> C[Asset Price Decline]
    C --> D[Collateral Value Impairment]
    D --> E[Forced Selling / Margin Calls]
    E --> F[Liquidity Stress]
    F --> G{Liquidity vs. Solvency?}
    G -->|Liquidity Shortfall| H[Fed Repo Operations / Discount Window]
    G -->|Potential Insolvency| I[Section 13(3) Facilities]
    H --> J[Federal Intervention Triggered]
    I --> J
```
...
```mermaid
flowchart TD
    A[Elevated Systemic Risk Indicators] --> B{Fed Assessment}
    B --> C[CCyB Activation]
    C --> D[Large Banks Build Capital Buffers]
    D --> E[Increased Loss Absorption Capacity]
    E --> F[Reduced Forced Deleveraging During Shock]
    F --> G[Lower Contagion Risk]
```
...
```mermaid
flowchart LR
    subgraph Leverage Accumulation
        A[Low rates enable leverage buildup] --> B[Duration mismatch in banks]
        A --> C[CRE debt accumulation]
        A --> D[Shadow banking expansion]
    end
    
    subgraph Shock Transmission
        D --> E[Rate shock triggers losses]
        E --> F[Unrealized losses materialize]
        F --> G[Capital erosion begins]
    end
    
    subgraph Intervention Trigger
        G --> H[Forced deleveraging]
        H --> I[Credit contraction / asset sales]
        I --> J[Contagion spreads]
        J --> K[Systemic risk threshold crossed]
        K --> L[Federal Intervention]
    end
```
...
```mermaid
flowchart TD
    A[Adverse Shock to<br/>Confidence] --> B[First-Mover Advantage<br/>Recognized]
    B --> C[Coordinated Withdrawal<br/>Pressure Begins]
    C --> D[Institution Faces<br/>Liquidity Demand]
    D --> E[Assets Sold at<br/>Fire Sale Prices]
    E --> F[Realized Losses<br/>Exceed Capital]
    F --> G[Solvency Question<br/>Becomes Real]
    G --> H[Contagion to<br/>Peer Institutions]
    H --> I[Systemic Crisis<br/>Requires Fed Intervention]
...
```mermaid
flowchart TD
    A[Asset Price Decline] --> B[Leveraged Institution Losses]
    B --> C[Margin Calls / Capital Erosion]
    C --> D[Forced Asset Sales]
    D --> E[Asset Price Amplification]
    E --> F[Counterparty Losses]
    F --> G[Funding Withdrawal]
    G --> H[Liquidity Crisis]
    H --> I[Fed Emergency Intervention Required]
    
    C --> J[Repo Market Stress]
    J --> H
    
    F --> K[Interbank Exposure]
    K --> F
```
...
```mermaid
flowchart TD
    A[Trigger Event] --> B{Major Bank Liquidity Stress?}
    B -->|Yes| C[Scenario A: Unlimited Repo Operations]
    B -->|No| D{Regional Bank Contagion?}
    D -->|Yes| E[Scenario B: BTFP-type Facility]
    D -->|No| F{Nonbank Financial Firm Failure?}
    F -->|Yes| G[Scenario C: Section 13(3) Backstop]
    F -->|No| H{Market Seizure?}
    H -->|Yes| G
    H -->|No| I[Monitor & Existing Facilities]
    
    C --> J{Continues Beyond 2027?}
    E --> J
    G --> J
    J -->|Yes| K[Fiscal Capacity Constraints?]
    K -->|Yes| L[Scenario D: Coordinated Response]
    K -->|No| M[Traditional Fed Backstop]
```
