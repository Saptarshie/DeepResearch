...
```mermaid
flowchart TD
    subgraph RISK_FACTORS["Risk Factor Accumulation"]
        A[Entitlement Spending Trajectory] --> D[Intervention Likelihood]
        B[Interest Burden Scaling] --> D
        C[NBFI Systemic Growth] --> D
        E[Geopolitical Shocks] --> D
...
```mermaid
flowchart TD
    A[Near-Term Risk Framework] --> B[Market Resilience Working Group]
    A --> C[Household Resilience Working Group]
    A --> D[AI Working Group]
    B --> B1[Treasury Markets]
    B --> B2[Short-term Wholesale Funding]
    B --> B3[Equity Markets]
    B --> B4[Credit Markets]
    C --> C1[Financial Shock Absorption]
    C --> C2[Essential Consumption Maintenance]
    C --> C3[Debt Cycle Avoidance]
    D --> D1[Opportunities in Financial Services]
    D --> D2[Risks in Financial Services]
```
...
```mermaid
flowchart TD
    A[Baseline: Elevated Debt + Interest Burden] --> B[Potential Trigger Events]
    B --> B1[Geopolitical Shock / Trade Conflict]
